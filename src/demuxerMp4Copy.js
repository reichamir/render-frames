...
export class MP4DemuxerCopy {
  ...
  constructor(uri, {onConfig, onChunk, setStatus}) {
    ...
    // this.fetchSegment(uri).then(initSegment => {
  ...
  appendFragment({payload, start}) {
    const segmentArrayBuffer = payload; //data.payload;
    segmentArrayBuffer.fileStart = start; // data.frag.start;  // Set the fileStart for each segment
    this.#file.appendBuffer(segmentArrayBuffer);
    this.#file.flush();
  }

  setInitSegment(data) {
    data.fileStart = 0;
      this.#file.appendBuffer(data);
      this.#file.flush();
  }
